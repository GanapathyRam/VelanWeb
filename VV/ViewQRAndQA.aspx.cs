...
namespace VV
{
    public partial class ViewQRAndQA : System.Web.UI.Page
    {
        ...
        protected void grdViewResult_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                DataSet ds = (DataSet)Cache["QRAndQAFromDBCache"];
            ...
            try
            {
                ...
                foreach (ListItem item in ChkOrderType.Items)
                {
                    if (item.Selected)
                    {
                       ...
                       // OrderType = OrderType + "'" + item.Value + "',";
                    }
                }
               ...
               // if(!String.IsNullOrEmpty(OrderType))
                ...
                searchDS = _dbObj.GetQRAndQAData(ChosenOrderType, Int32.Parse(txtWeekNo.Text.Trim()));

                Cache["QRAndQAFromDBCache"] = searchDS;

                int EndWeek = Int32.Parse(txtWeekNo.Text.Trim());
                EndWeek = EndWeek + 3;

                if (searchDS != null && searchDS.Tables[0].Rows.Count > 0)
                    this.grdViewResult.Columns[3].HeaderText = "Week " + txtWeekNo.Text.Trim() + " - " + EndWeek.ToString();
