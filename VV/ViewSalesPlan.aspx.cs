...
namespace VV
{
    public partial class ViewSalesPlan : System.Web.UI.Page
    {
        ...
        protected void grdViewSalesPlan_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                DataSet ds = (DataSet)Cache["SalesDataFromDBCache"];

                grdViewSalesPlan.PageIndex = e.NewPageIndex;
                grdViewSalesPlan.DataSource = ds;
                grdViewSalesPlan.DataBind();
            }
        ...
        protected void btnMacro_Click(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect("~/ViewMacroSalesPlan.aspx?WeekNo=" + (String)Cache["WeekNo"].ToString() + "");
            }
            catch (Exception ex)
            {
