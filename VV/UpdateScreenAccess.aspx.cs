...
namespace VV
{
    public partial class UpdateScreenAccess : System.Web.UI.Page
    {
        ...
        protected void btnSearchBox_Click(object sender, EventArgs e)
        {
            try
            {
                String EmpName_Search = txtEmpName.Text.Trim();
                DataSet searchDS = new DataSet();

                DBUtil _DBUtil = new DBUtil();
                searchDS = _DBUtil.GetScreenAccessInfoForEmployee(EmpName_Search);

                grdViewScreenResult.DataSource = searchDS;
                grdViewScreenResult.DataBind();
        ...
        protected void btnGiveAccess_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                foreach (GridViewRow row in grdViewScreenResult.Rows)
                {
                    bool isChecked = ((CheckBox)row.FindControl("chkSelect")).Checked;
                    if (isChecked)
                    {
                        String ScreenAccessID = ((System.Web.UI.HtmlControls.HtmlInputHidden)row.FindControl("hidScreenAccessID")).Value.ToString();
                        _DBObj.UpdateScreenAccess(Int32.Parse(ScreenAccessID), true);
                    }
                    else
                    {
                        String ScreenAccessID = ((System.Web.UI.HtmlControls.HtmlInputHidden)row.FindControl("hidScreenAccessID")).Value.ToString();
                        _DBObj.UpdateScreenAccess(Int32.Parse(ScreenAccessID), false);
                    }
                }
            }
            catch (Exception ex)
            {
                Logger.Write(this.GetType().ToString() + " : btnGiveAccess_Click : " + " : " + DateTime.Now + " : " + ex.Message.ToString(), Category.General, Priority.Highest);
                throw ex;
            }
        }
    }
