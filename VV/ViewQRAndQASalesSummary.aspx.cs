...
namespace VV
{
    public partial class ViewQRAndQASalesSummary : System.Web.UI.Page
    {
        ...
        protected void grdViewResult_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                DataSet ds = (DataSet)Cache["QRAndQASalesSummaryFromDBCache"];

                grdViewResult.PageIndex = e.NewPageIndex;
                grdViewResult.DataSource = ds;
                grdViewResult.DataBind();   
            }

            catch (Exception ex)
            {
                Logger.Write(this.GetType().ToString() + " : grdViewResult_PageIndexChanging : " + " : " + DateTime.Now + " : " + ex.Message.ToString(), Category.General, Priority.Highest);
                throw ex;
            }
        }
        ...
        protected void btnSearchBox_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                String ChosenOrderType = String.Join(",", YrStrList.ToArray());

                searchDS = _dbObj.GetQRAndQASalesSummaryData(ChosenOrderType, Int32.Parse(txtWeekNo.Text.Trim()));

                Cache["QRAndQASalesSummaryFromDBCache"] = searchDS;

                int StartWeek = Int32.Parse(txtWeekNo.Text.Trim());
                int EndWeek = StartWeek + 3;

                if (searchDS != null && searchDS.Tables[0].Rows.Count > 0)
                {
                    this.grdViewResult.Columns[3].HeaderText = "Week " + StartWeek.ToString() + " - " + EndWeek.ToString();

                    StartWeek = EndWeek + 1;
                    EndWeek = StartWeek + 3;
                    this.grdViewResult.Columns[4].HeaderText = "Week " + StartWeek.ToString() + " - " + EndWeek.ToString();

                    StartWeek = EndWeek + 1;
                    EndWeek = StartWeek + 3;
                    this.grdViewResult.Columns[5].HeaderText = "Week " + StartWeek.ToString() + " - " + EndWeek.ToString();
                }

                grdViewResult.DataSource = searchDS;
                grdViewResult.DataBind();
            }
            catch (Exception ex)
            {
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Convert(ds, "QR And QA Sales Summary");
            }
            catch (Exception ex)
            {
