...
namespace VV
{
    public partial class Shortage : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = (DataSet)Cache["CacheFromShortageItems"];
            ...
            {
                LogError(ex, "Exception from while clicking on page number from Shortage Items.");
            }
        }
        ...
        public void showgrid()
        {
            try
            {
                ...
                DataSet ds = _DBObj.RetriveByShortageItems();

                ds = GetFormattedDataSet(ds);

                Cache["CacheFromShortageItems"] = ds;
        ...
        protected void btnSearchBox_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(searchRowFilter))
            {
                //DataSet searchDS = (DataSet)Cache["CacheFromShortageItems"];
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                cmd = new SqlCommand("[spGetShortageDetails]", conn);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "ShortageDetails.xls"));
        ...
        private DataSet GetFormattedDataSet(DataSet ds)
        {
            int SoNoPrevious = 0;
            int PosPrevious = 0;
            int rowNo = 0;
            ...
            {
                SoNoPrevious = Convert.ToInt32(ds.Tables[0].Rows[0]["SONo"].ToString());
                PosPrevious = Convert.ToInt32(ds.Tables[0].Rows[0]["Pos"].ToString());

                do
                {
                    int SoNoCurrent = Convert.ToInt32(ds.Tables[0].Rows[rowNo]["SONo"].ToString());
                    int PosCurrent = Convert.ToInt32(ds.Tables[0].Rows[rowNo]["Pos"].ToString());

                    if (SoNoPrevious != SoNoCurrent || PosPrevious != PosCurrent)
                    {
                        ds.Tables[0].Rows.InsertAt(ds.Tables[0].NewRow(), rowNo);
                        rowNo++;
                        SoNoPrevious = SoNoCurrent;
