...
namespace VV
{
    public partial class SalesOrderQtyDecrement : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ShowSaleOrderQtyDecrementDetails();
            }
        ...
        protected void GridView1_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            ...
            DataSet ds = (DataSet)Cache["CacheFromSaleOrderQtyDecrementDetails"];
        ...
        private void ShowSaleOrderQtyDecrementDetails()
        {
            try
            {
                ...
                DataSet ds = _DBObj.RetriveBySaleOrderQtyDecrement();
                Cache["CacheFromSaleOrderQtyDecrementDetails"] = ds;
            ...
            {
                LogError(ex, "Exception from display Sale Order Quantity Decrement Details!");
            }
        }
        ...
        {
            txtQtyToBeReduced.Text = string.Empty;
            lblMessage.Visible = false;
        }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string qtyToBeReduced = txtQtyToBeReduced.Text;
                ...
                foreach (GridViewRow row in GridView1.Rows)
                {
                    ...
                    if (isChecked)
                    {
                        ...
                        string lblReleaseQty = ((Label)row.FindControl("lblReleaseQty")).Text.ToString();
                        string lblBalanceQty = ((Label)row.FindControl("lblBalanceQty")).Text.ToString();
                        string lblOrderQty = ((Label)row.FindControl("lblQrderQty")).Text.ToString();

                        int quantityToBeReduced = Convert.ToInt32(lblOrderQty) - Convert.ToInt32(qtyToBeReduced);

                        if (Convert.ToInt32(quantityToBeReduced) <= Convert.ToInt32(lblReleaseQty))
                        {
                            _DBObj.UpdateSaleOrderQtyDecrement(Convert.ToInt32(quantityToBeReduced), Convert.ToInt32(lblOrderNo),
                                Convert.ToInt32(lblPos));
                        ...
                        {
                            lblMessage.Text = "Cannot update!. Please try with other order no.";
                            lblMessage.Visible = true;
                        }
        ...
        protected void btnSearchBox_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!String.IsNullOrEmpty(searchRowFilter))
                {
                    DataSet searchDS = (DataSet)Cache["CacheFromSaleOrderQtyDecrementDetails"];
                ...
                else if (string.IsNullOrEmpty(searchRowFilter1) && string.IsNullOrEmpty(searchRowFilter2))
                {
                    DataSet searchDS = (DataSet)Cache["CacheFromSaleOrderQtyDecrementDetails"];
