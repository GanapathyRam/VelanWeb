...
namespace VV
{
    public partial class UploadUpdatedMISSalesInput : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = (DataSet)Cache["ImportUpdatedMISSalesInputCache"];
            ...
            {
                Logger.Write(this.GetType().ToString() + " : Upload Updated MIS Sales Input - When trying to do a Paging : " + " : " + DateTime.Now + " : " + ex.Message.ToString(), Category.General, Priority.Highest);
            }
        }
        ...
        protected void UploadButton_Click(object sender, EventArgs e)
        {
            try
            {
                if (FileUploader.HasFile)
                {
                    ...
                    UpdateRowsInMISSalesInput(ds);

                    DBUtil _dbObj = new DBUtil();

                    DataSet ds_FromDB = _dbObj.GetAllMISSalesInput();

                    Cache["ImportUpdatedMISSalesInputCache"] = ds_FromDB;

                    grdSearchResult.DataSource = ds_FromDB;
                    grdSearchResult.DataBind();
                }
            }
            catch (Exception ex)
        ...
        private void UpdateRowsInMISSalesInput(DataSet ds_Uploaded)
        {
            try
            {
                DBUtil _dbObj = new DBUtil();
                DataSet ds_FromDB_B4Update = _dbObj.GetAllMISSalesInput();
                ...
                for (int i = 0; i < ds_Uploaded.Tables[0].Rows.Count; i++)
                {
                    ...
                    # region Data To be Updated
                    String SAPCodeFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["SAPCode"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["SAPCode"].ToString().Trim();
                    String StockCodeFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["StockCode"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["StockCode"].ToString().Trim();
                    String TAGNoFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["TagNo"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["TagNo"].ToString().Trim();
                    String GADNoFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["GADNo"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["GADNo"].ToString().Trim();
                    String QCINoFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["QCINo"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["QCINo"].ToString().Trim();

                    String O2FromExcel = ds_Uploaded.Tables[0].Rows[i]["O2"].ToString().Trim().ToUpper().Equals("YES") ? "Y" : String.Empty;
                    String H2FromExcel = ds_Uploaded.Tables[0].Rows[i]["H2"].ToString().Trim().ToUpper().Equals("YES") ? "Y" : String.Empty;
                    String IBRFromExcel = ds_Uploaded.Tables[0].Rows[i]["IBR"].ToString().Trim().ToUpper().Equals("YES") ? "Y" : String.Empty;
                    String ASUFromExcel = ds_Uploaded.Tables[0].Rows[i]["ASU"].ToString().Trim().ToUpper().Equals("YES") ? "Y" : String.Empty;

                    String RelDateFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["RelDate"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["RelDate"].ToString().Trim();

                    String NaceFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["Nace"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["Nace"].ToString().Trim();
                    String LowEmissionFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["LowEmission"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["LowEmission"].ToString().Trim();
                    String OtherRequirementsFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["OtherRequirements"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["OtherRequirements"].ToString().Trim();
                    String GADDateFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["GADDate"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["GADDate"].ToString().Trim();
                    String PODateFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["PODate"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["PODate"].ToString().Trim();
                    String ValueTypeFromExcel = String.IsNullOrEmpty(ds_Uploaded.Tables[0].Rows[i]["ValueType"].ToString().Trim()) ? String.Empty : ds_Uploaded.Tables[0].Rows[i]["ValueType"].ToString().Trim();
                    ...
                    if (Filter.Length > 0)
                    {
                        isRecExistsinDB = true;
                    }
                    else
                    {
                        isRecExistsinDB = false;
                    }
            ...
            {
                Logger.Write(this.GetType().ToString() + " : UpdateRowsInMISSalesInput : " + " : " + DateTime.Now + " : " + ex.Message.ToString(), Category.General, Priority.Highest);
                throw ex;
            }
        }
