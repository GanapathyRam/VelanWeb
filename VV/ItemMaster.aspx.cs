    ...
    public partial class ItemMaster : System.Web.UI.Page
    {
        ...
        {            
            try
            {
                ...
                _dbObj.InsertIntoItemMaster(itemName.Trim().ToLower(), units.Trim().ToLower());
            ...
            {
                LogError(ex, "Exception occured -- insert item master details.");
            }
            finally
            {
            }
        ...
        protected void btnSearchBox_Click(object sender, EventArgs e)
        {
            ...
            string ItemName = txtItemNameSearch.Text.Trim().ToLower();

            if (!string.IsNullOrEmpty(ItemName))
            {
                searchRowFilter4 = "ItemName like '" + ItemName + "%'";
            }
            ...
            if (!String.IsNullOrEmpty(searchRowFilter))
            {
                DataSet searchDS = (DataSet)Cache["CacheFromItemMasterDetails"];
        ...
        protected void GridView1_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            ...
            DataSet ds = (DataSet)Cache["CacheFromItemMasterDetails"];
            ...
            {
                LogError(ex, "Exception from while clicking on page number from item master.");
            }
        }
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            try
            {
                ...
                int itemCode = Convert.ToInt32(GridView1.DataKeys[e.RowIndex].Values[0]);
                string itemName = (row.Cells[2].Controls[0] as TextBox).Text;
                string units = ((TextBox)(row.Cells[3].Controls[0])).Text;

                _dbObj.UpdateIntoItemMaster(Convert.ToString(itemCode), itemName.Trim().ToLower(), units.Trim().ToLower());
            ...
            {
                LogError(ex, "Exception from while updating row from Item Master.");
            }
        }
        ...
        //        _dbObj.DeleteIntoItemMaster(Convert.ToString(ItemCode));

        //        ShowItemMasterDetails();
        //    }
        //    catch (Exception ex)
        //    {
        //        LogError(ex, "Exception from while deleting row from Item Master.");
        //    }
        //}
        ...
        private void ShowItemMasterDetails()
        {
            try
            {
                ...
                DBUtil _DBObj = new DBUtil();
                DataSet ds = _DBObj.RetriveByItemMasterDetails();

                Cache["CacheFromItemMasterDetails"] = ds;
            ...
            {
                LogError(ex, "Exception from view item master details!");
            }
        }
    }
}