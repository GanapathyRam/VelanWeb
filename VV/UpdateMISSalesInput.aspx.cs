...
namespace VV
{
    public partial class UpdateMISSalesInput : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    DataSet ds = _dbObj.GetMISSalesInput(Convert.ToString(orderNo), LineNo, Int32.Parse(Pos));
                    Cache["IsDataExists"] = "F";

                    if (ds != null)
                    {
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            Cache["IsDataExists"] = "T";

                            txtSAPCode.Text = ds.Tables[0].Rows[0]["SAPCode"].ToString().Trim();
                            txtStockCode.Text = ds.Tables[0].Rows[0]["StockCode"].ToString().Trim();

                            if (ds.Tables[0].Rows[0]["O2"].ToString().Trim().Equals("Y"))
                                chkO2.Checked = true;
                            else
                                chkO2.Checked = false;

                            if (ds.Tables[0].Rows[0]["H2"].ToString().Trim().Equals("Y"))
                                chkH2.Checked = true;
                            else
                                chkH2.Checked = false;

                            if (ds.Tables[0].Rows[0]["IBR"].ToString().Trim().Equals("Y"))
                                chkIBR.Checked = true;
                            else
                                chkIBR.Checked = false;

                            if (ds.Tables[0].Rows[0]["ASU"].ToString().Trim().Equals("Y"))
                                chkASU.Checked = true;
                            else
                                chkASU.Checked = false;
        ...
        protected void buttonUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                String IBR = String.Empty;
                String ASU = String.Empty;
                ...
                if (chkH2.Checked == true)
                    H2 = "Y";

                if (chkIBR.Checked == true)
                    IBR = "Y";

                if (chkASU.Checked == true)
                    ASU = "Y";

                bool isDataExists = false;

                if (Cache["IsDataExists"].ToString().Trim().Equals("F"))
                    isDataExists = false;
                else
                    isDataExists = true;
                ...
                {
                    // Update
                    _DBObj.UpdateMISSalesInput(Convert.ToString(lblOrderNoVal.Text.Trim()), lblLineNumVal.Text.Trim(), Int32.Parse(lblPosVal.Text.Trim()), SAPCode, StockCode, O2, H2, IBR, ASU, String.Empty, String.Empty, String.Empty, String.Empty, String.Empty, String.Empty, String.Empty, String.Empty, String.Empty, String.Empty);
                }
                else
                {
                    // Insert
                    _DBObj.InsertIntoMISSalesInput(Convert.ToString(lblOrderNoVal.Text.Trim()), lblLineNumVal.Text.Trim(), Int32.Parse(lblPosVal.Text.Trim()), SAPCode, StockCode, O2, H2, IBR, ASU, String.Empty, String.Empty, String.Empty, String.Empty,
                        String.Empty, String.Empty, String.Empty, String.Empty, String.Empty, String.Empty);
                }


                Cache.Remove("IsDataExists");
        ...
        {
            Response.Redirect("ViewMISSalesInput.aspx");
        }
    }
}